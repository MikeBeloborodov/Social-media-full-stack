...
def postgres_database_connection() -> list:
    while True:
        try:
            ...
            print(f'[{time_stamp()}][+] DB CONNECTION WAS SUCCESSFUL')
            return [connection, cursor]
        except Exception as db_connection_error:
            print(f'[{time_stamp()}][!] DB CONNECTION ERROR: {db_connection_error}')
            print(f"[{time_stamp()}][!] TRYING TO RECONNECT AGAIN EVERY 5 SECONDS...")
            time.sleep(5)
...
def return_all_posts(connection, cursor, user_id) -> list:
    ...
    if not posts:
        ...
        print(f"[{time_stamp()}][+] SENDING ALL POSTS FROM DB")
        return posts
...
def return_post_by_id(connection, cursor, id, user_id) -> list:
    ...
    if not post:
        ...
        print(f"[{time_stamp()}][+] SENDING POST WITH ID {id}")
        return post
...
def save_post_to_db(connection, cursor, new_post: NewPost, user_id) -> list:
    ...
    try:
        ...
        print(f"[{time_stamp()}][!] COULD NOT CREATE NEW POST (FAILED TO RETRIEVE USER FROM DB): {execution_error}")
        connection.rollback()
        raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not write post to DB, user id retreival error.")

    if not posting_user:
            print(f"[{time_stamp()}][!] TRIED TO CREATE A NEW POST, BUT POSTING USER WAS NOT FOUND, ID = {user_id}")
    ...
    try:
        ...
        connection.commit()
        print(f"[{time_stamp()}][+] CREATED NEW POST - {new_post.title}")
...
def save_updated_post_by_id(connection, cursor, id, updated_post: UpdatedPost, user_id) -> list:
    ...
    try:
        ...
        print(f"[{time_stamp()}][!] COULD NOT EXTRACT POST TO UPDATE, DB ERROR: {execution_error}")
        connection.rollback()
        raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not find post to update in DB")

    if not found_post:
        print(f"[{time_stamp()}][!] USER ID = {user_id} TRYING TO UPDATE A NON EXISTING POST")
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="This post does not exist")
    ...
    try:
        ...
        print(f"[{time_stamp()}][!] COULD NOT UPDATE POST (FAILED TO RETRIEVE USER FROM DB): {execution_error}")
        connection.rollback()
    ...
    if not updating_user:
            print(f"[{time_stamp()}][!] TRIED TO UPDATE POST, BUT UPDATING USER WAS NOT FOUND, ID = {user_id}")
            raise HTTPException(status.HTTP_403_FORBIDDEN, detail="User with this id does not exist")
    ...
    try:
        ...
        print(f"[{time_stamp()}][!] COULD NOT UPDATE POST: {evaluation_error}")
    ...
    try:
        ...
        returning_post = cursor.fetchone()
        connection.commit()
        print(f"[{time_stamp()}][+] UPDATED POST FROM USER ID {user_id} - {updated_post.title}")
        return returning_post
    except Exception as execution_error:
...
def delete_post_from_db(connection, cursor, id, user_id) -> list:
    ...
    if not found_post:
        print(f"[{time_stamp()}][!] USER ID {user_id} TRYING TO DELETE A NON EXISTING POST")
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="This post does not exist")
    ...
    try:
        ...
        print(f"[{time_stamp()}][!] COULD NOT DELETE POST (FAILED TO RETRIEVE USER FROM DB): {execution_error}")
        connection.rollback()
    ...
    if not deleting_user:
            print(f"[{time_stamp()}][!] TRIED TO DELETE POST, BUT USER WAS NOT FOUND, ID = {user_id}")
            raise HTTPException(status.HTTP_403_FORBIDDEN, detail="User with this id does not exist")
    ...
    try:
        ...
        connection.commit()
        print(f"[{time_stamp()}][+] DELETED POST FROM USER ID {user_id} - POST ID {id}")
        return deleted_post
    except Exception as execution_error:
        print(f"[{time_stamp()}][!] COULD NOT DELETE POST: {execution_error}")
        connection.rollback()
...
def save_post_like_to_db(connection, cursor, id: int, user_id) -> list:
    ...
    try:
        ...
        print(f"[{time_stamp()}][!] COULD NOT LIKE POST: {execution_error}")
        connection.rollback()
    ...
    try:
        ...
        connection.commit()
        print(f"[{time_stamp()}][+] POST ID {id} WAS LIKED BY USER ID {user_id}")
        return returning_post
    except Exception as execution_error:
        print(f"[{time_stamp()}][!] COULD NOT LIKE POST: {execution_error}")
        connection.rollback()
...
def save_user_to_db(connection, cursor, new_user: CreateUser) -> list:
    ...
    try:
        ...
        connection.commit()
        print(f"[{time_stamp()}][+] CREATED NEW USER - {new_user.email}")
        return user
    except Exception as execution_error:
...
def check_user_credentials(connection, cursor, user_credentials: LoginUser) -> dict:
    ...
    try:
        ...
        print(f"[{time_stamp()}][!] UNABLE TO RETRIEVE USER DURING LOGIN - {user_credentials.username}")
        print(f"[{time_stamp()}]Error: {hash_error}")
        connection.rollback()
